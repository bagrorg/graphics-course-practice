...
void main()
{
	...
	vec3 normal_raw = texture(normal_texture, texcoord).rgb * 2.0 - vec3(1.0);
	vec3 real_normal = normalize(mix(normal, tbn * normal_raw, 0.5));

	vec3 ref_dir = reflect(camera_dir, real_normal);	

	float x = atan(ref_dir.z, ref_dir.x) / PI * 0.5 + 0.5;
	float y = -atan(ref_dir.y, length(ref_dir.xz)) / PI + 0.5;
	...
	vec3 res = 0.5 * (albedo + env_albedo);

    out_color = vec4(lightness * res, 1.0);
}
)";
...
void main()
{
    ...
    vec4 ndc = vec4(vs[gl_VertexID], 0.0, 1.0);
    vec4 clip_space = view_projection_inverse * ndc;
    position = clip_space.xyz / clip_space.w;

    texcoord = (vs[gl_VertexID] + 1.0) / 2.0;
    gl_Position = vec4(vs[gl_VertexID], 0.0, 1.0);
}
)";
...
void main()
{
    ...
    float x = atan(camera_dir.z, camera_dir.x) / PI * 0.5 + 0.5;
    float y = -atan(camera_dir.y, length(camera_dir.xz)) / PI + 0.5;
...
int main() try
{
    ...
    auto vertex_shader_env = create_shader(GL_VERTEX_SHADER, env_vertex_shader_source);
    auto fragment_shader_env = create_shader(GL_FRAGMENT_SHADER, env_fragment_shader_source);
    ...
    GLuint ENV_view_location = glGetUniformLocation(program_env, "view");
    GLuint ENV_projection_location = glGetUniformLocation(program_env, "projection");
    GLuint ENV_camera_position_location = glGetUniformLocation(program_env, "camera_position");
    GLuint ENV_env_texture_location = glGetUniformLocation(program_env, "env_texture");
    ...
    while (running)
    {
        ...
        glUniform1i(ENV_env_texture_location, 2);
